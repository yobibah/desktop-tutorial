<link rel="stylesheet" href="../css/pieds.css">

<footer>

    <p>Copyright © 2024 BECCOD-INTERNATIONAL. Tous droits réservés.
        <div class="ligne"></div>
        <div class="place">
        <div class="listes">
        <ul>
            <li class="nav-item  <?php if ($nav==='accueil'):?>active <?php endif ; ?>"><a href="../apres_con.php" >Accueil</a></li>
            <li class="nav-item  <?php if ($nav==='services'):?>active <?php endif ; ?>"><a href="service.php">Nos services</a></li>
            <li class="nav-item  <?php if ($nav==='consultation'):?>active <?php endif ; ?>"><a href="consultation.php" >Consultation</a></li>
            <li class="nav-item  <?php if ($nav==='a propos'):?>active <?php endif ; ?>"><a href="#">À propos</a></li>
        </ul>
        </div>
        <div class="donnes">
            <ul>
                <li><a href="contact.php">X</a></li>
                <li><a href="contact.php">whatsapp</a></li>
                <li><a href="contact.php">email</a></li>
                <li><a href="contact.php">telephone</a></li>

            </ul>
            
        </div>
    
        </div>

        <div class="ligne"></div>
        <div class="icone">
            <img src="../images/logo.jpg" alt="">
        </div>
</footer>